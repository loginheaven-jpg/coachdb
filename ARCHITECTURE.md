...
10. [역량 템플릿 아키텍처](#역량-템플릿-아키텍처-2026-02-06)
...
#### 2.2. 2-tier 통합 템플릿 아키텍처 (Template-Based Architecture)
...
## 2-tier 통합 템플릿 아키텍처 (2026-02-05)

### 개요

역량항목의 입력과 평가를 통합한 2계층 템플릿 시스템:
...
│                                                                     │
│  unified_template_id: "kca_certification"  ← 입력+평가 통합        │
│  evaluation_method_override: null | "by_name" | "by_existence"     │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  UnifiedTemplate (통합 템플릿)                                       │
│                                                                     │
│  ┌── 입력 설정 ──────────────────┐  ┌── 평가 설정 ─────────────────┐│
│  │ - fields_schema              │  │ - grade_type: string        ││
│  │ - layout_type                │  │ - matching_type: grade      ││
│  │ - is_repeatable              │  │ - aggregation_mode          ││
│  │ - data_source                │  │ - default_mappings          ││
│  │ - help_text                  │  │ - evaluation_method         ││
│  └──────────────────────────────┘  └─────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────┘
```
...
**3-tier → 2-tier 마이그레이션 (2026-02-05)**:
- **이전**: InputTemplate + ScoringTemplate + CompetencyItem (3개 테이블)
- **현재**: UnifiedTemplate + CompetencyItem (2개 테이블)
- **이유**: 입력 템플릿과 평가 템플릿이 대부분 1:1 관계이므로 통합
- **특수 케이스**: 자격증은 `evaluation_method`로 평가 방법 선택 가능 (by_name / by_existence)

### UnifiedTemplate (통합 템플릿)

폼 입력 구조와 점수 계산 방식을 함께 정의합니다.
...
- `is_repeatable`: 복수 입력 가능 여부
- `help_text`: 입력 안내 메시지

**평가 설정 필드:**
- `evaluation_method`: 평가 방법
  - `standard` (기본): 일반 평가
  - `by_name`: 이름 기준 평가 (CONTAINS 매칭) - 자격증용
  - `by_existence`: 유무 기준 평가 (FILE_EXISTS) - 자격증용
- `grade_type`: 등급 유형 (string, numeric, file_exists, multi_select)
- `matching_type`: 매칭 방식 (exact, contains, range, grade)
- `aggregation_mode`: 복수입력 집계 방식 (first, sum, max, best_match)
- `default_mappings`: JSON 배열 - 등급→점수 매핑
- `proof_required`: 증빙 필수 여부 (not_required, optional, required)

**기본 제공 템플릿 (9개):**
| template_id | 이름 | 평가 | 비고 |
|-------------|------|------|------|
| text | 텍스트 | 없음 | 단순 입력 |
| number | 숫자 | 없음 | 단순 입력 |
| select | 단일선택 | 없음 | 단순 입력 |
| file | 파일 | 없음 | 파일만 |
| degree | 학위 | grade | 박사/석사/학사 |
| coaching_history | 코칭이력 | 없음 | 정성 평가 |
| coaching_time | 코칭시간 | range+sum | 시간 합산 |
| coaching_experience | 코칭경력 | range+sum | 시간 합산 |
| kca_certification | KCA자격증 | grade | 고정 등급 |
| certification | 자격증 | **by_name/by_existence** | 선택 가능 |
| text_file | 텍스트+파일 | 없음 | 범용 |

### 자격증 평가 방법 선택

자격증 템플릿은 두 가지 평가 방법 중 선택 가능:

**1. 이름으로 평가 (by_name)**:
- 자격증명에 특정 키워드 포함 여부로 점수 부여
- matching_type: `contains`
- 예: "임상심리사" 포함 → 30점

**2. 유무로 평가 (by_existence)**:
- 증빙 파일 첨부 여부만 확인
...
CompetencyItem(
    ...
    evaluation_method_override="by_name"  # 기본값 사용 시 null
)
```

### 역량항목-템플릿 매핑

**API 엔드포인트**: `POST /api/admin/link-unified-templates`

| 역량항목 코드 | 통합 템플릿 | 평가 방법 오버라이드 |
|-------------|-----------|-------------------|
...
| EXP_COACHING_HOURS | coaching_time | - |
| EXP_COACHING_TRAINING | coaching_experience | - |
| COACHING_BUSINESS | coaching_history | - |
| COACHING_CAREER | coaching_history | - |

### 위저드 자동 설정 로드

과제 생성 위저드 Step4에서:
1. 선택된 역량항목의 `unified_template_id` 확인
2. 역량항목의 `evaluation_method_override` 적용
3. 해당 템플릿의 `default_mappings` 자동 로드
4. 과제관리자는 점수만 조정하면 됨
...
**통합 템플릿 관리**:
- `GET /api/unified-templates` - 목록 조회
- `GET /api/unified-templates/{id}` - 상세 조회
- `GET /api/unified-templates/{id}/effective-scoring?evaluation_method=by_name` - 실제 적용될 평가 설정
- `POST /api/unified-templates` - 생성 (SUPER_ADMIN)
- `PUT /api/unified-templates/{id}` - 수정 (SUPER_ADMIN)
- `DELETE /api/unified-templates/{id}` - 비활성화 (SUPER_ADMIN)

**시드 API**:
- `POST /api/admin/seed-unified-templates?secret_key=coachdb-seed-2024` - 기본 템플릿 삽입
- `POST /api/admin/link-unified-templates?secret_key=coachdb-seed-2024` - 역량항목 연결

### 관리 UI

- **통합 템플릿 관리**: `/admin/competency-items` (템플릿 관리 탭)
- **역량항목 관리**: `/admin/competency-items` (역량항목 관리 탭)
  - 템플릿 선택 시 자격증인 경우 평가방법 라디오 버튼 표시

### Legacy 호환성

기존 `input_template_id`, `scoring_template_id` 컬럼은 유지되지만 deprecated:
- 신규 항목은 `unified_template_id` 사용 권장
- 점진적 마이그레이션 지원

---

## 역량 템플릿 아키텍처 (2026-02-06)

### 개요

**역량 템플릿(CompetencyItem)**: 기존 "역량항목"을 "역량 템플릿"으로 재정의.
모든 평가/입력 설정이 역량 템플릿에 완전 통합되어, 프리셋(UnifiedTemplate)은 생성 시 기본값 제공 역할만 담당.
...
│  역량 템플릿 생성 시 기본값 제공용. 저장 후 연결 의미 없음             │
│                                                                     │
│  - fields_schema (필드 구조)                                         │
│  - layout_type, is_repeatable, aggregation_mode                     │
│  - validation_rules                                                 │
│  - default_mappings (기본 등급-점수 매핑 → 복사 원본)                │
│  - 모든 평가/소스 설정은 역량 템플릿에 복사 후 독립                   │
└─────────────────────────────────────────────────────────────────────┘
                              │
                              │ "복사 후 독립" (생성 시 1회 복사)
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Tier 2: CompetencyItem (SUPER_ADMIN) — "역량 템플릿"                │
│  저장 후 프리셋과 독립. 수퍼어드민은 모든 항목을 자유롭게 편집.       │
│  grade_edit_mode는 과제관리자의 제약만 정의. 수퍼어드민 자신은 무제약. │
│                                                                     │
│  ┌── 평가 설정 (Phase 4) ────┐  ┌── 점수 소스 (Phase 5) ───────────┐│
│  │ - grade_type              │  │ - scoring_value_source            ││
│  │ - matching_type           │  │ - scoring_source_field            ││
│  │ - grade_edit_mode         │  │ - extract_pattern                 ││
│  │ - evaluation_method       │  └───────────────────────────────────┘│
│  │ - data_source             │                                       │
│  │ - has_scoring (computed)  │  ┌── 복사 후 독립 ──────────────────┐│
│  └───────────────────────────┘  │ - grade_mappings                 ││
│                                  │ - proof_required                 ││
│  ┌── 역량 템플릿 전용 ──────┐   │ - help_text                      ││
│  │ - verification_note      │   │ - placeholder                    ││
│  │ - auto_confirm_across_   │   └───────────────────────────────────┘│
│  │   projects               │                                       │
...
└─────────────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Tier 3: ProjectCompetencyMapping (PROJECT_MANAGER)                 │
│  "프로젝트별 추가 설정" - grade_edit_mode 범위 내에서만 수정 가능     │
│                                                                     │
│  - max_score (최대 점수)                                             │
│  - is_required (필수 여부)                                           │
│  - scoring_criteria (점수 기준) ← grade_edit_mode에 따라 제한        │
└─────────────────────────────────────────────────────────────────────┘
```

### 핵심 원칙: "복사 후 독립"

역량 템플릿 생성 시 프리셋의 기본값을 복사하고, 이후에는 독립적으로 관리됩니다:

1. **복사 시점**: CompetencyItem 생성 시 프리셋의 default_mappings → grade_mappings로 복사
2. **독립 관리**: 복사 후에는 역량 템플릿이 프리셋과 독립적으로 관리됨
3. **프리셋 수정 영향 없음**: 프리셋 수정이 기존 역량 템플릿에 영향을 주지 않음
4. **Phase 5 완전 독립**: scoring_value_source, scoring_source_field, extract_pattern도 역량 템플릿에서 직접 관리

```python
# 역량항목 생성 시 템플릿 기본값 복사
item = CompetencyItem(
    item_code="CERT_NEW",
    unified_template_id="certification",
    # 템플릿에서 복사된 필드들
    grade_mappings=template.default_mappings,  # 복사
    proof_required=template.proof_required,     # 복사
    help_text=template.help_text,               # 복사
    placeholder=template.placeholder,           # 복사
    # 역량항목 전용 필드
    verification_note="검증 안내 문구",
...
| 분류 | 필드 | 위치 | 수정 권한 |
|------|------|------|----------|
| **프리셋 전용** | fields_schema, layout_type, is_repeatable, aggregation_mode, validation_rules | UnifiedTemplate | SUPER_ADMIN만 |
| **평가 설정 (Phase 4)** | grade_type, matching_type, grade_edit_mode, evaluation_method, data_source, has_scoring(computed) | CompetencyItem | SUPER_ADMIN (제약 없음) |
| **점수 소스 (Phase 5)** | scoring_value_source, scoring_source_field, extract_pattern | CompetencyItem | SUPER_ADMIN (제약 없음) |
| **복사 후 독립** | grade_mappings, proof_required, help_text, placeholder | CompetencyItem | SUPER_ADMIN (제약 없음) |
| **역량 템플릿 전용** | verification_note, auto_confirm_across_projects, field_label_overrides | CompetencyItem | SUPER_ADMIN (제약 없음) |
...
- `KCA 자격증`: KSC(30), KPC(20), KAC(10) 고정

### KCA 인증자격 역량 템플릿 (특수 패턴)

코치 가입 시 입력한 `coach_certification_number` (예: KPC03669)에서 자동 판별:
...
proof_required: not_required       # 사용자 프로필에서 자동 읽기
```

**특징**: 코치가 직접 입력할 필드 없음 (fields: []). 사용자 프로필의 인증번호에서 자동으로 등급과 점수가 결정됨.
- `유무 평가`: 유자격(20), 무자격(0) 고정

### DB 스키마 변경 (competency_items)

```sql
ALTER TABLE competency_items ADD COLUMN IF NOT EXISTS
    -- 템플릿에서 복사 후 독립 관리
    grade_mappings TEXT DEFAULT '[]',           -- 등급-점수 매핑 (JSON)
    proof_required VARCHAR(20) DEFAULT 'optional',
    help_text TEXT,
    placeholder VARCHAR(200),
    ...
    -- Phase 4: 평가 설정 (역량 템플릿 완전 독립화 - 2026-02-06)
    ...
    -- Phase 5: 점수 소스 설정 (프리셋에서 완전 독립 - 2026-02-06)
    scoring_value_source VARCHAR(50) DEFAULT 'submitted', -- submitted, user_field, json_field
    scoring_source_field VARCHAR(100),                    -- User 필드명 (예: coach_certification_number)
    extract_pattern VARCHAR(200);                         -- 정규식 추출 패턴 (예: ^(.{3}))
```
...
# 2. 기본값 복사 (명시적 값이 없는 경우 - 저장 후 프리셋과 독립)
item = CompetencyItem(
    # 복사 후 독립 필드
    ...
    # Phase 5: 점수 소스 설정도 프리셋에서 복사 후 독립
    scoring_value_source=data.scoring_value_source or template.scoring_value_source or 'submitted',
    scoring_source_field=data.scoring_source_field or template.scoring_source_field,
    extract_pattern=data.extract_pattern or template.extract_pattern,
)
```

### 프론트엔드 UI (좌우 분할 레이아웃)

**역량 템플릿 관리** (`/admin/competency-items`) - 탭 구성:
- **역량템플릿** 탭: 역량 템플릿 목록 관리 (생성/수정/삭제)
- **프리셋** 탭: UnifiedTemplate 프리셋 관리 (기본값 제공용)

**역량 템플릿 편집/생성 모달** - 1000px 좌우 분할:

**상단**: 프리셋 선택 (선택 시 아래 필드에 기본값 자동 채움, 저장 후 연결 의미 없음)
...
- **등급 매핑 편집기**: grade_type에 따라 동적 UI
  - file_exists: Select(있음/없음), numeric: InputNumber, string: Input
  - 수퍼어드민은 모든 모드에서 자유 편집 가능
- **점수 소스 설정** (Phase 5):
  - 값 소스 (scoring_value_source): submitted, user_field, json_field
  - 소스 필드명 (scoring_source_field): user_field/json_field일 때만 표시
  - 추출 패턴 (extract_pattern): 정규식 패턴

### 권한 체계 요약

| 역할 | 프리셋 | 역량 템플릿 | 프로젝트 항목 설정 |
|------|--------|-----------|------------------|
| SUPER_ADMIN | 생성/수정/삭제 | 생성/수정/삭제 (무제약) | 생성/수정/삭제 |
